...
// ---------------------------------------------------------------------------
// File-private entry type — mirrors AeroNodeEntry / ChannelPickerScreen.Option.
// ---------------------------------------------------------------------------

private data class SensorNodeEntry(val node: Node, val compatible: Boolean)
 ...
 * `block_sensor` node to wire the source reading into.
 *
 * Lists all [Node]s in [target]'s graph whose [Node.typeKey] path is
 * `"block_sensor"`. Each row:
 ...
 */
class SensorTargetPickerScreen(
    private val target: LogicBlockEntity,
    private val sourcePinType: PinType,
    private val onPick: (NodeId) -> Unit,
) : NwComposeScreen(Component.literal("Select block_sensor node")) {

    private val entries: List<SensorNodeEntry> = target.graph.nodes.values
        .filter { it.typeKey.path == "block_sensor" }
        .map { node ->
            val outType = node.outputs.firstOrNull()?.type
            SensorNodeEntry(node, compatible = outType == sourcePinType)
        }
...
@Composable
private fun TargetNodeRow(entry: SensorNodeEntry, sourcePinType: PinType, onClick: () -> Unit) {
    ...
    val label = entry.node.label?.takeIf { it.isNotBlank() } ?: "block_sensor"
    ...
    Row(
        ...
        Box(
            modifier = Modifier
                .size(6)
                .background(
                    if (compatible) sensorPinColor(dotType) else NwTheme.colors.onSurfaceMuted,
